            ...
            <div class="pt-[55px] sm:pt-[130px] mb-[40px] sm:mb-[72px] gap-[28px] flex flex-col lg:flex-row justify-between items-left">
                ...
                <div class="flex items-center gap-[12px] bg-white border border-light-grey rounded-[6px] w-full max-w-sm h-[50px] sm:w-[322px] px-[12px]">
                    ...
                    <form action="/available-books" method="get">
                        <label for="book-search" class="sr-only">Rechercher un livre</label>
                        ...
                        <div class="flex flex-col items-left w-[160px] sm:w-[200px] sm:h-[324px] shadow-[0px_4px_24px_0px_rgba(0,0,0,0.01)]">
                            <div class="size-[160px] sm:size-[200px]">
                                ...
                                <?php if ($book->getAvailability()->value === 'NOT-AVAILABLE'): ?>
                                    <div class="relative top-[-150px] sm:top-[-184px] left-[80px] sm:left-[118px] font-inter font-medium leading-none tracking-normal text-[8px] text-center text-white w-[71px] h-[18px] pt-[5px] px-[13px] rounded-[30px] <?= ($book->getAvailability()->value === 'AVAILABLE') ? 'bg-greendispo' : 'bg-red' ?>"><?= ($book->getAvailability()->value === 'AVAILABLE') ? 'disponible' : 'non dispo.' ?></div>
                                <?php endif ?>
                            </div>

                            <div class="pt-[16px] sm:pt-[20px] pb-[19px] sm:pb-[23px] pl-[11px] sm:pl-[14px] rounded-b-[15px] bg-white">
                                <p class="font-inter font-normal leading-none tracking-normal text-[13px] sm:text-[16px] text-black"><?= $book->securePrintText($book->getTitle(), 22) ?></p>
                                <p class="pt-[7px] sm:pt-[8px] font-inter font-normal leading-none tracking-normal text-[11px] sm:text-[14px] text-grey"><?= $book->securePrintText($book->getAuthor(), 20) ?></p>
                                <p class="pt-[19px] sm:pt-[22px] font-inter italic font-normal leading-none tracking-normal text-[8px] sm:text-[10px] text-grey"><em>Vendu par : <?= $book->securePrintText($book->getFromMember()->getUsername(), 20) ?></em></p>
                            </div>
                        </div>
                    </a>
                <?php endforeach ?>
